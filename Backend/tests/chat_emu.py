    ...
    async def pressButton(self, user: str, callbackData: str, msg_id: int):
        ...
        try:
            if callbackData == 'report-csv':
                await self.callbackFacade.report_csv_callback(msg, dbs)
            elif callbackData == 'cancel-send':
                await self.callbackFacade.cancel_callback(msg, dbs)
            elif callbackData == 'reset-costs':
                await self.callbackFacade.reset_callback(msg, dbs)
